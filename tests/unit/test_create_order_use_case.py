...
def test_creates_order_with_correct_total(use_case):
    input_dto = CreateOrderInput(
        customer_id="cust-1",
        items=[
            CreateOrderItemInput(product_id="sku-1", quantity=2, unit_price=Decimal("10.00")),
            CreateOrderItemInput(product_id="sku-2", quantity=1, unit_price=Decimal("5.50")),
        ],
    )
...
def test_rejects_negative_quantity(use_case):
    input_dto = CreateOrderInput(
        customer_id="cust-1",
        items=[CreateOrderItemInput(product_id="sku-1", quantity=-1, unit_price=Decimal("1.00"))],
    )
...
def test_persists_order_so_it_can_be_retrieved():
    ...
    input_dto = CreateOrderInput(
        customer_id="cust-1",
        items=[CreateOrderItemInput(product_id="sku-1", quantity=1, unit_price=Decimal("2.00"))],
    )
